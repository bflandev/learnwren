...
>
> Not built yet: module / course completion rollups (rest of EP-06), the rest of the mutating half of Manage Users (US-08-01 Slices C/D — suspend, delete + anonymise) and the remaining EP-08 admin stories (Manage Categories, Monitor Platform Health). US-08-03 (admin instructor-application review), US-08-01 Slice A (read-only admin user directory) and US-08-01 Slice B (promote/demote) are shipped. EP-07 (Instructor Dashboard) is fully shipped. `docs/USER_GUIDE.md` is the authoritative end-to-end feature matrix.
...
│   ├── web-admin/           # Angular admin surface: /admin/instructor-applications queue (US-08-03), /admin/users directory + promote/demote (US-08-01 A+B)
...
| `web-admin` | Library | Angular standalone admin surface: the `/admin/instructor-applications` review queue (US-08-03) + the `/admin/users` user directory (`AdminUsersPageComponent` + `AdminUserDetailPageComponent`, US-08-01 Slice A) with promote/demote role actions on the detail page (Slice B) |
