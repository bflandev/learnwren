...
3. **SMTP transport never sets `secure`/`requireTLS`**: STARTTLS on 587 was only opportunistic (downgrade-stripping surfaces as a confusing auth failure). *(Corrected during execution: nodemailer 8.0.7 does infer implicit TLS on 465 when `secure` is undefined, so the original "465 hangs" claim was wrong — the genuine fix is mandatory STARTTLS; explicit `secure` is best-practice hardening.)* → §1.3 fix.
